...
class SANet:
    ...
    def __init__(self, content, style, channels, scope='attention'):
        with tf.variable_scope(scope):
            f = conv(content, channels // 8, kernel=1, stride=1, scope='f_conv') # [bs, h, w, c']
            g = conv(style,   channels // 8, kernel=1, stride=1, scope='g_conv') # [bs, h, w, c']
            h = conv(style,   channels     , kernel=1, stride=1, scope='h_conv') # [bs, h, w, c]
